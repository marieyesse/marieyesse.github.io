...
FINEanim = ggplot()+
 ...
 theme_bw() +
  geom_label(data=FINE[Region=="EU28" & Year %in% c(2010:2100)], aes(x=Scenario, y=value, label=substr(Carrier,1,3), group=label, fill=Carrier), size=3.5, colour="black",  position=position_stack(vjust=0.5)) +
...
CALCALLRES15=data.table(CALCALLRES15)
CALCALLRES15 <- na.omit(CALCALLRES15)

DECARBplot = ggplot() +
    geom_point(data=CALCALLRES15, aes(x=EI, y=EE, colour=IPCC), size=2, alpha=0.6) +

          annotate("rect", xmin=-Inf,xmax=Inf,ymin=min(CALCALLRES15[Year=="2050" & IPCC %in% c("Below 2C", "1.5C or below"), c( "EE")], na.rm = FALSE),ymax=max(CALCALLRES15[Year=="2050" & IPCC %in% c("Below 2C", "1.5C or below"), c( "EE")], na.rm = FALSE), fill="lightgrey", alpha=0.4) +
  
            annotate("rect", ymin=-Inf, ymax=Inf, xmin=min(CALCALLRES15[Year=="2050" & IPCC %in% c("Below 2C", "1.5C or below"), c( "EI")], na.rm = FALSE),xmax=max(CALCALLRES15[Year=="2050" & IPCC %in% c("Below 2C", "1.5C or below"), c( "EI")], na.rm = FALSE), fill="lightgrey", alpha=0.4) +
  
   annotate("text", x=5.5, y=-5, label="2050 solution space", size=5, angle=0, colour="dimgrey",  hjust=0) +
