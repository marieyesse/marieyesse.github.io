...
SCENall <- rbindlist(SCENSdat_list, fill=TRUE) %>%
    drop_na(Model) %>%
    gather(Year, value, -c(Model, Scenario, Region, Variable, Unit)) %>%
    mutate(Year=as.numeric(Year), value=as.numeric(value)) %>%
    mutate(Scenario=gsub(".xlsx", "", Scenario, fixed=T)) %>%
    drop_na(Year) %>%
        bind_rows({.} %>% filter(Region %in% c("WWorld", "CWorld")) %>% 
                    group_by(Model, Scenario, Variable, Unit, Year) %>% 
                    summarise(value=sum(value)) %>%
                    mutate(Region="World")) %>%
  filter(!Region == "World") %>%
    bind_rows({.} %>% filter(!Region %in% c( "World")) %>% 
              group_by(Model, Scenario, Variable, Unit, Year) %>% 
              summarise(value=sum(value)) %>%
              mutate(Region="World")) %>%
  filter( !Scenario %in% c("SSP2_450","SSP2_MaxTax"))#"SSP2_MaxTax", 

SCENdat <- SCENall %>% 
  filter( Scenario %in% c("TechReplace", "DemandMan", "CircEco", "ProcEff"))
...
```{r econ-emis-ghg, echo=FALSE, results=TRUE, message=FALSE, warning=FALSE, fig.width=9, fig.height=5}

ECONGHG <- SCENdat %>%
    filter(grepl("Emissions|Kyoto Gases", Variable, fixed=T)) 
...
<!-- ## Industrial decarbonization overview {.tabset} -->

<!-- ```{r all-emis, echo=FALSE, results=TRUE, message=FALSE, warning=FALSE, fig.width=9, fig.height=9} -->

<!-- SCENINDUSALL <- SCENall %>% -->
<!--     filter(str_detect(tolower(Variable), c("steel", "cement", "paper",  "food")))  -->

<!-- SCENINDUSALLEMIS <-   SCENINDUSALL %>% -->
<!--     filter(grepl("emissions", tolower(Variable), fixed=T)) %>% -->
<!--     filter(!grepl("cokes", tolower(Variable), fixed=T)) %>% -->
<!--       filter(grepl("Direct emissions", Variable, fixed=T)) %>% -->
<!--     bind_rows(SCENall %>% -->
<!--             filter(str_detect(tolower(Variable),"chemicals and petrochemicals")) %>% -->
<!--              filter(grepl("Direct emissions", Variable, fixed=T)) %>% -->
<!--             filter(grepl("hvc", tolower(Variable))) -->
<!--        ) %>% -->
<!--   mutate(Scengroup=case_when(grepl("Circ", Scenario) ~"CircEco", -->
<!--                              grepl("Demand", Scenario) ~"DemandMan", -->
<!--                              grepl("Proc", Scenario) ~"ProcEff", -->
<!--                              grepl("Tech", Scenario) ~"TechReplace")) %>% -->
<!--     mutate(ScenLabel=case_when(grepl("4000USD", Scenario) ~"FSC infeasible ", -->
<!--                              grepl("450", Scenario) ~"new SSP2_450 ctax", -->
<!--                              grepl("MaxTax", Scenario) ~"MaxTax ctax", -->
<!--                              TRUE ~"MaxTax (TIMER_2015)")) %>% -->
<!--     separate(Variable, c("X", "Y", "Industry", "Type"), sep = "[|]") %>% -->
<!--     filter(Region=="World" ) %>% -->
<!--     ggplot()+ -->
<!--     geom_line(aes(x=Year, y=value, colour=ScenLabel, group=Scenario)) + -->
<!--      labs(title="CO2 Emissions", -->
<!--         x="Year", -->
<!--         y="MtCO2/yr") + -->
<!--     facet_grid(Industry~Scengroup, scales="free") + -->
<!--          theme(axis.text.x = element_text(angle=45, hjust=1), -->
<!--                text = element_text(size=14)) -->

<!--  ggplotly(SCENINDUSALLEMIS) -->
...
```{r ce-emis, echo=FALSE, results=TRUE, message=FALSE, warning=FALSE, fig.width=9, fig.height=7}
...
SCENCHEMEMIS <-   SCENCHEM %>%
 ...
 SCENCHEMEMIS2 <-   SCENCHEM %>%
    ...
    filter(grepl("direct", tolower(Variable), fixed=T))  %>%
    separate(Variable, c("X", "Y", "Industry", "Type", "Z"), sep = "[|]") %>%
    filter(Region=="World" & Z=="Direct emissions" ) %>%
   ggplot()+
...
<!-- ```{r food-agriemis, echo=FALSE, results=TRUE, message=FALSE, warning=FALSE, fig.width=9, fig.height=5} -->
...
<!--     "AGMIP|OAP|PROD", -->
<!--     "AGMIP|LSP|PROD") ) %>% -->
<!--     mutate(Food=case_when(grepl("DRY", Variable)~"Dairy", -->
<!--                           grepl("RUM", Variable)~"Ruminant meat", -->
<!--                           grepl( "NRM", Variable)~"Non-ruminant meat", -->
<!--                           grepl( "RIC", Variable)~"Rice", -->
<!--                           grepl( "WHT", Variable)~"Wheat", -->
<!--                           grepl( "CGR", Variable)~"Coarse grains", -->
<!--                           grepl( "OSD", Variable)~"Oil Seeds", -->
<!--                           grepl( "SGC", Variable)~"Sugar crops", -->
<!--                           grepl( "VFN", Variable)~"Vegetables, Fruits & Nuts", -->
<!--                           grepl( "PFB", Variable)~"Plant-based fibres", -->
<!--                           grepl( "ECP", Variable)~"Energy crops", -->
<!--                           grepl( "OCR", Variable)~"Other crops", -->
<!--                           grepl( "CRP", Variable)~"All crops", -->
<!--                           grepl( "OAP", Variable)~"Other animal products", -->
<!--                           grepl( "LSP", Variable)~"Livestock", -->
<!--                           TRUE ~ "Total")) %>% -->
<!--   mutate( Variable=gsub("Agricultural Demand|", "", Variable, fixed=T)) %>% -->
<!--   separate(col=Variable, sep="([\\|\\?\\:])", into=c("Project", "Commodity", "Indicator"), remove=F) %>% -->
<!--   mutate(Type=ifelse(Indicator=="PROD", "Production", "Emissions")) %>% -->
<!--   select(Commodity, Variable, Scenario, Region, Unit, Year, Project, Indicator ,Type, value, Food) %>% -->
<!--   bind_rows({.} %>%  -->
<!--               filter(Type=="Emissions") %>% -->
<!--               group_by(Commodity, Scenario, Region, Unit, Year, Project,  Food) %>% -->
<!--               summarise(value=sum(value)) %>% -->
<!--                           mutate(Type = "GHG Emissions", -->
<!--                                  Indicator="GHG Emissions", -->
<!--                                  Variable="GHG Emissions"))  -->

<!-- SCENFOODAGREMIS <- SCENFOODAGR %>% -->
<!--   # mutate(Region=ifelse(Region=="World", "RoW", Region)) %>% -->
<!--   # full_join({.} %>% filter(Region=="RoW") %>% -->
<!--   #             rename("WORLDval"="value", -->
<!--   #                    "WORLD"="Region"), by=c("Commodity", "Year", "Variable", "Scenario", "Unit", "Project", "Indicator", "Type", "Food")) %>% -->
<!--   # mutate(WORLDshare=value/WORLDval*100, Label=paste(Scenario, Region, Variable)) %>% -->
<!--   filter(Type %in% c( "GHG Emissions") & Scenario=="Baseline" & Commodity %in% c("DRY", "RUM", "NRM", "VFN")) %>% -->
<!--   ggplot()+ -->
<!--   geom_area(data=. %>% filter(Region=="RoW"), aes(x=Year, y=value, group=Label, fill=Region), alpha=1, colour="black") + -->

<!--   geom_area(data=. %>% filter(!Region=="RoW" ), aes(x=Year, y=value, group=Label, fill=Region), alpha=1, colour="black") + -->
<!--   facet_grid(Type~Food, scales="free_y") + -->
<!--   labs(y="Total emissions (MtCO2e)", -->
<!--        x="Year") -->


<!--  ggplotly(SCENFOODAGREMIS) -->
