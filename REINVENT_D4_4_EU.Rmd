...
*	**Chemicals:** Carbon Looping for CO2 does not appear (Winner takes all logit parameter?)
*	**Chemicals:** Final Energy does not account for the biomass energy requirements used in BLF/BSF (for heat)
...
SCENCHEMEMIS <-   SCENCHEM %>%
    ...
    filter(Region=="EU" & Type=="Direct emissions" ) %>%
    ggplot()+
    geom_line(aes(x=Year, y=value, colour=Scenario, group=Scenario)) +
     labs(title="CO2 Emissions",
        x="Year",
